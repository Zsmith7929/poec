...
def _row(tid: str, ev_net: float, deep_link: str | None = None) -> EvRow:
    return EvRow(
        table_id=tid,
        name=tid,
...
def test_insert_and_recent_round_trip(tmp_path) -> None:
    ...
    repo.insert_many(
        ...
        "sha256:abc",
        [
            _row("a", 90.0, deep_link=_VERIFY_URL),
            _row("b", 5.0),
        ],
    )
    ...
    by_tid = {r["table_id"]: r for r in recent}
    # row with deep_link: URL round-trips correctly
    assert by_tid["a"]["deep_link"] == _VERIFY_URL
    # row without deep_link: deep_link is None
